...
Working with
[React Components](https://reactjs.org/docs/components-and-props.html) in a
project I frequently come across things that are kind of like a component that
already exists, but not 100% the same. Usually the data or presentation have
slight differences. When dealing with code, this leads to an interesting problem
of deciding between adjusting the existing component to be re-used for this
slightly different purpose or copy/pasting the existing component and ending up
with 2 similar components instead.
...
For 'copy/pasting the existing component' that usually means taking what was
...
Both of these methods (`reuse` and `duplicate`) involve a decent amount of work,
but they're usually less than starting from scratch on the piece when there's a
similar component in the project.

Thanks to source control software like Git, it's usually relatively painless to
switch approaches if I find that I picked the wrong one to start with. The
biggest downside of pivoting to the other option is the lost time, which is a
...
spend the minimum required time on the feature), I usually dive into the code
and look around to see what indicators exist in the situation that would make
one path better or easier than the other. Before I get into those indicators let
me share an example from a recent project.
