...
pretty much every release of the libraries I follow there's at least some sort
...
project! From what I've seen, there's about 4 categories you could split into:
...
it's better to have a small update reguarly instead of waiting until there's a
...
unless they're minor and easy. Then, once you've got an update that looks
useful/major/helpful add a task to do it and test after. In the case of
applications that aren't worked on often, I've also seen that there's very few
resources available to test that it still works and that makes updating really
difficult. More on that later.

For libraries I've found that as often as there are updates to dependencies it's
worth at least looking into updating to use it. In most cases there's benefit to
updating and releasing a version since those updates will include bug fixes that
you might not have run into but your consumers might.
...
help getting people to update (I know I do!). Outside of automated tooling,
you'll want to make sure that you read through any provided documentation like
changelogs or release notes that can give you insight into things you might need
to update.
...
thinking, there's a few reasons for this:
...
complex, or there's too many things that need updates, or you simply don't have
time to test all the things in your application that could potentially be
...
were updating the babel 7, but they did it at different times and so it was
